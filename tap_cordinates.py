class Coordinates:
    ...
    def get_coordinates(self, category, number):
        ...
        if category in self.__dict__:
            ...
            if 'x' in coords and 'y' in coords:
                x = coords['x'][number - 1] if number <= len(coords['x']) else None
                y = coords['y']
                if x is not None:
                    return (x,y)
            else:
    ...
    def get_noble_phantasm(self, number):
        # Returns the (x, y) coordinates for 
    ...
    def get_master_skills(self):
        return(self.master_skills[0], self.master_skills[1])
